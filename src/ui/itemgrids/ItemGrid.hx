...
import ui.itemgrids.itemtiles.ItemTile;
import ui.itemgrids.itemtiles.ItemTileEvent;
import ui.tooltip.EquipmentToolTip;
import ui.view.Inventory;
import util.NativeTypes;

class ItemGrid extends Sprite {
	...
	private function onTileMove(e: ItemTileEvent) {
		...
		if (Std.isOfType(target, InteractiveItemTile)) {
	...
	private function onCtrlClick(e: ItemTileEvent) {
		...
		if (Std.isOfType(tile.ownerGrid, InventoryGrid)) {
			slot = tile.ownerGrid.curPlayer.swapInventoryIndex();
			if (slot != -1) {
